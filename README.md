...
本のタイトルを入力して追加し、「読了」/「未読に戻す」でステータスを切り替える静的単一ページアプリ。不要な本は一覧から削除でき、直後なら取り消せる。一覧は localStorage（キー `reading-list-books`）に `{ id, title, done }` の配列として保存され、リロード後も復元される。空・空白のみのタイトルは追加されない。読了の本は打ち消し線と淡色で区別し、一覧が空のときは「まだ本がありません」を表示する。
...
- 各本の「削除」で一覧から取り除く。直後なら「元に戻す」で同じ位置・同じ読了状態に復元できる（リロード後は取り消せない）
